...
\end{itemize}
This correction ensures accurate readings by accounting for thermal expansion effects.\\[6pt]
\begin{minipage}{0.65\textwidth}
	\textbf{2. Open-Tube Manometer (Differential Pressure)}\\[8pt]
	The open-tube manometer, typically in a U-tube configuration, measures pressure differences by comparing the fluid levels in both arms of the tube. The pressure difference is given by:  
	\begin{equation}
		P_1 - P_2 = (\rho_m - \rho_f)gh
	\end{equation}
	For gases, where the fluid density $\rho_f$ is much smaller than the manometric fluid density $\rho_m$, this simplifies to:  
	\begin{equation}
		\Delta P \approx \rho_mgh
	\end{equation}
	When one side of the manometer is open to the atmosphere, it directly measures gauge pressure, meaning that $P_2$ is simply the atmospheric pressure.
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}\centering
	...
	\end{tikzpicture}
	\captionof{figure}{Open manometers (Izaak Neutelings, 2021)}
\end{minipage}\\[6pt]
The sensitivity of the device can be enhanced in several ways. One approach is to use an \textbf{inclined tube}, where the pressure difference relates to the fluid displacement along the tube length $L$ at an angle $\theta$:  
\begin{equation}
	\Delta P = \rho gL\sin\theta
\end{equation}
Another method is to use a lower-density fluid, such as water, which has a density of 998 kg/m$^3$. This allows for greater displacement per unit pressure difference, improving measurement precision.\\
\begin{minipage}{0.65\textwidth}
	\textbf{3. Sealed-Tube Manometer (Absolute Pressure)}\\[8pt]
	The sealed-tube manometer is designed for absolute pressure measurement. Unlike open-tube configurations, it has one end closed, providing a fixed reference point—either a vacuum or a gas of known pressure. The pressure is determined by:  
	\begin{equation}
		P = \rho g h
	\end{equation}
	A key advantage of this setup is that it eliminates the influence of atmospheric pressure, making it particularly useful for measuring vacuum pressures.\\[8pt]
	In some designs, well-type sealed manometers are used to enhance precision. These variants account for differences in the cross-sectional areas of the tube and the well.
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}\centering
	\begin{tikzpicture}[scale=1.4,transform shape]
		\def\W{0.8} \def\H{0.7} \def\Rx{0.55} \def\Ry{0.2*\Rx}
		...
		\draw[<->] (0,\hL) -- (0,\hR) node[midway,fill=white,inner sep=1] {$h$};
	\end{tikzpicture}
	\captionof{figure}{Sealed manometers (Izaak Neutelings, 2021)}
\end{minipage}\\[8pt]
...
\begin{center}
	\begin{tblr}{
		...
		\SetCell{bg=black!10!white,fg=black}Parameter & Torricelli & Open-Tube & Sealed-Tube & Inclined & Digital \\
...
\begin{center}
	...
	\end{minipage}
\end{center}
\captionof{table}{Consistent Gradient Values Across Units, Mean = $\pm 0$}
...
\vspace{0.2em}
	...
	\end{minipage}	
\end{center}
\captionof{table}{Percent errors for Different Gauges across all units}\label{table:Percenterrors}
\vspace{1em}
...
The percentage errors observed across the different pressure-measuring devices reveal clear discrepancies in their performance, particularly between pressurized and vacuum conditions. While some gauges maintained reasonable accuracy, others exhibited substantial deviations, highlighting potential issues related to their design, calibration, and handling.\\[8pt]
Different pressure gauges rely on distinct operating principles, which inherently affect their accuracy under varying conditions. Without a precise understanding of the \textbf{specific type of each gauge}, it is difficult to determine whether the observed errors are due to instrument limitations or external factors.\\[8pt]
However the experimental results demonstrate that all gradient errors are \textbf{unit-invariant}, confirming that the observed deviations reflect intrinsic instrument characteristics and {systematic errors} rather than unit conversion artifacts.\\[8pt]
Summarized Table \ref{table:Percenterrors}:\\
\begin{enumerate}
\item {Bourdon Gauge 1 (psi gauge, range: -30 to 20)}
\begin{itemize}
	\item Pressurized accuracy of \textbf{2.24\%} suggests reasonable performance.
	\item Extreme vacuum error of \textbf{-47.59\%} indicates poor reliability under vacuum conditions.
\end{itemize}
\item {Bourdon Gauge 2 (kN/m$^2$ gauge, range: -101 to 120)}
\begin{itemize}
	\item Pressurized error of \textbf{-5.38\%} and vacuum error of \textbf{-4.91\%} indicate consistent accuracy.
	\item Cumulative error of \textbf{-5.17\%} suggests this gauge maintains calibration well.
\end{itemize}
\item {Budenberg Gauge (bar gauge, range: -1 to 2.5)}
\begin{itemize}
	\item Highly accurate, with errors of \textbf{-0.68\%} (pressurized) and \textbf{-5.84\%} (vacuum).
	\item Cumulative error of \textbf{-0.59\%} confirms reliability.
\end{itemize}
\item {Mercury (Hg) Glass Manometer (cm Hg, range: -40 to 40)}
\begin{itemize}
	\item Extreme errors: \textbf{94.88\%} (pressurized) and \textbf{106.79\%} (vacuum).
\end{itemize}
\end{enumerate}

\subsection{Critical Examination of Mercury Manometer Methodology}
I want to first talk about this first big discrepancy, the results obtained from the mercury manometer experiment reveal a significant disconnect between the recorded data and the actual pressure measurements, in so requiring a thorough analysis of both procedural shortcomings and measurement principles. The experimental approach suffered from a critical oversight: the "cm Hg" values were recorded based purely on height observations from only one side of the U-tube, and no calculations for pressure ($\Delta P$) were performed. Proper manometer operation requires differential measurement of both columns to calculate the pressure difference. This oversight led to several primary issues:
\begin{itemize}
	\item \textbf{Misinterpreted Quantities}: The recorded heights were assumed to directly correspond to pressure values, but in reality, they represented only half of the necessary measurement. This assumption led to inaccurate conclusions about the pressure. The correct interpretation should have been that the difference in the heights ($h_1 - h_2$) is proportional to the pressure difference, in so which is to be later calculated in the report.
	\item \textbf{Incomplete Data Collection}: The datasheet only provided space for recording measurements from one column, omitting the second essential column needed for accurate pressure calculation. As a result, the data was insufficient to properly determine the pressure difference. The pressure difference ($\Delta P$) should have been calculated as:
	\[\Delta P = \rho g (h_1 - h_2)\]
	where $\rho$ is the density of the mercury, $g$ is the acceleration due to gravity, and $h_1$ and $h_2$ represent the heights of the mercury columns in the U-tube.
	\item \textbf{Unrecoverable Data Limitations}: The recorded measurements could not be reliably converted into pressure values because the reference point for the height measurements was not specified, the second column's measurements were missing, and no directional information was provided to indicate which side was measured.
\end{itemize}
In so subsequent analysis, which involved zeroing the data, converting to consistent units, and applying a linear regression for best-fit analysis, ended up being based on the height measurement ($h_1$) rather than the actual pressure difference ($\Delta P$). As a result, the final pressure values were incorrect. While students are responsible for understanding measurement principles, the experimental setup contained significant flaws that compromised the accuracy of the results.\\[8pt]
This experience underscores the importance of ensuring the accuracy of measurements and the completeness of data collection. Future experiments should emphasize clarity in what physical quantities are being recorded and ensure that all required measurements are properly documented. In addition, experimental protocols should provide clear instructions on how to handle the equipment and offer sufficient guidance on measurement procedures to avoid confusion.\\[8pt]
Despite this, it was possible to reverse-engineer the data and predict the second column's value, $h_2$, by making reasonable assumptions about the expected percentage error. This reverse engineering approach demonstrated that the total difference in height ($\Delta h$) should be approximately double the value of the observed $h_1$. This prediction aligns reasonably with what we would expect based on the instrument's behavior and suggests that the discrepancy in results was indeed a result of human error, not any fault in the manometer itself.
...
While the mercury manometer data cannot be salvaged for proper pressure analysis, this examination serves to:

\begin{itemize}
	\item Document the specific measurement challenges encountered
	\item Explain the physical reasons for the observed discrepancies
	\item Provide recommendations for improving future experiments
\end{itemize}

The case underscores how critical proper experimental design is for obtaining valid measurements, particularly when using fundamental instruments where the connection between observation and derived quantity is absolute.

The percentage errors highlight significant differences in performance. The Budenberg gauge demonstrated the best accuracy, while Bourdon Gauge 2 performed reliably. Bourdon Gauge 1 struggled with vacuum measurements, and the Hg glass manometer was highly unreliable. 

Crucially, \textbf{not knowing the specific type of each gauge} limits definitive conclusions. The absence of details such as whether gauges were \textbf{absolute or gauge pressure devices, their intended applications, and their mechanical response properties} makes it difficult to separate instrument faults from experimental conditions or human errors.

Ultimately, this experiment emphasizes the importance of proper \textbf{gauge selection, calibration, and handling} to ensure accurate pressure measurements.
