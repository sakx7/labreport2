...
\end{center}
   ...
   \newpage
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{Elongation at Break \& Percentage elongation}
            \item \textbf{Ratios}
        \end{itemize}
    \end{itemize}
   ...
   \subsection{Hardness}\label{hardnesstypes}
    ...
    \begin{minipage}{0.46\textwidth}
            ...
            \wm{\item \(A\): Initial cross-sectional area (metres squared, \(\text{m}^2\))}
            \end{itemize}
        \end{minipage}\hfill
        \begin{minipage}{0.47\textwidth}
            ...
            \end{equation}
            \begin{itemize}[left=0pt,itemsep=-1mm]
                \wm{\item \(A\): Initial cross-sectional area (meters squared, \(\text{m}^2\))}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    Given the relation \( \sigma = E \varepsilon \), stress (\( \sigma \)) is directly proportional to strain (\( \varepsilon \)) were the proportionality constant is Young's modulus (\(E\)). This concept is further expanded in the Hooke's Law section.\\[8pt]
...
The \textbf{Elastic Limit (Yield Strength)} is important in determining the maximum allowable load a material can bear without permanent deformation. It is distinct from the \textit{ultimate tensile strength} (UTS), which is the maximum load a material can withstand before failure. For ductile materials, the ratio of yield strength to UTS is often used in applications like pipelines and is proportional to the strain hardening exponent shown in the ratio section.
...
Ultimate Tensile Strength (UTS) is quite straightforward in its theoretical background is not quite as extensive as others, it represents the maximum stress a material can withstand while being stretched before it breaks.\\

The maximum stress applied in tension to a material before rupture is defined:
\begin{equation}
    P_{max} = \text{Ultimate Tensile Strength (UTS)} \times A
\end{equation}
%where, P_max = Maximum Tensile Force
...
It is one of the most important properties for assessing a material's ability to resist failure under tensile loads. UTS is determined from a stress-strain curve as the \textbf{peak stress value} before fracture. This property is critical for ensuring safety in applications like cables, bridges, and load-bearing components.  
...
%    \subsection{Poisson's Ratio}
    ...
    \begin{itemize}
        ...
        \item \(F\) vs \(\Delta L\)
    \end{itemize}
    We can now derive several mechanical properties, these derived properties will help us fully characterize the mechanical performance of the material under stress.
    \newpage
    ...
    \subsection{Force vs Displacement}
        ...
        \begin{table}[H]
            ...
            \begin{tblr}{
                ...
                AR & 24400 & 9.7 \\
                \end{tblr}
            \caption{Relevant extracted data machine table (See Figure \ref{fig:tensile_results})}
